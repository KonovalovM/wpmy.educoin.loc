...
<!-- Is coure homeworks checkings allowed? -->
<?php if ( !$tmpl_vars['is_course_homeworks_disabled'] && 
           !$tmpl_vars['is_user_course_homeworks_disabled'] ) { ?>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <form class="" action="<?=get_permalink()?>" method="post">
                    ...
                    <input type="hidden" name="task" value="done_course_lesson_howemork">
                    <input type="hidden" name="action" value="save">
                    <input type="hidden" name="lesson_id" value="<?=$tmpl_vars['lesson_id']?>">
                    ...
                    <div class="text-center">
                        ...
                        <?php 
                            ...
                            // is lesson completed?
                            if ( !empty( $tmpl_vars['lesson_progress']['is_accepted'] ) ) {
                        ?>

                                <button type="submit" name="btn_send_for_check" class="btn btn-secondary mb-4 mt-4 mx-auto" disabled><?=edc__( 'Урок пройден' )?></button>

                        <?php 
                            } 
                            // is lesson shoud be checked?
                            else if ( $tmpl_vars['lesson_progress']['is_need_to_check'] ) {
                        ?>

                                <button type="submit" name="btn_send_for_check" class="btn btn-secondary mb-4 mt-4 mx-auto" disabled><?=edc__( 'Ожидает проверки' )?></button>

                        <?php 
                            } else if ( $tmpl_vars['passing_status'] == 'closed' ) {
                        ?>

                                <button type="submit" name="btn_send_for_check" class="btn btn-secondary mb-4 mt-4 mx-auto" disabled><?=edc__( 'Урок закрыт' )?></button>

                        <?php 
                            } else if ( $tmpl_vars['is_course_ended'] ) {
                        ?>

                                <button type="submit" name="btn_send_for_check" class="btn btn-secondary mb-4 mt-4 mx-auto" disabled><?=edc__( 'Курс закончился' )?></button>

                        <?php 
                            } else {
                        ?>

                                <button type="submit" name="btn_save" class="btn btn-primary mb-4 mt-4 mx-auto"><?=edc__( 'Сохранить' )?></button>
                                <button type="submit" name="btn_send_for_check" class="btn btn-primary mb-4 mt-4 mx-auto"><?=edc__( 'Выслать на проверку' )?></button>
